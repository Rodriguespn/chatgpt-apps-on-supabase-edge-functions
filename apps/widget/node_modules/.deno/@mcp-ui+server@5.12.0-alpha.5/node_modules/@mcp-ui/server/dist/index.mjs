const g = "mcpui.dev/ui-", p = `"use strict";
(() => {
...
function o(e) {
  const s = { ...e.resourceProps ?? {} };
  if (e.uiMetadata || e.metadata) {
    const t = Object.fromEntries(
      Object.entries(e.uiMetadata ?? {}).map(([a, n]) => [
        `${g}${a}`,
        n
      ])
    );
...
function c(e) {
  ...
  if (typeof TextEncoder < "u" && typeof btoa < "u") {
    ...
    for (let r = 0; r < t.length; r += n)
      a += String.fromCharCode(...t.slice(r, r + n));
    return btoa(a);
  } else {
    console.warn(
...
function m(e, s) {
  if (!s)
    return { htmlContent: e };
  const t = e, a = [];
  let n;
  if (s.appsSdk?.enabled) {
    const d = l(s.appsSdk.config);
    a.push(d), n || (n = s.appsSdk.mimeType ?? "text/html+skybridge");
  }
  if (a.length === 0)
    return { htmlContent: e };
  const r = a.join(`
`);
  let i;
  return t.includes("<head>") ? i = t.replace("<head>", `<head>
${r}`) : t.includes("<html>") ? i = t.replace("<html>", `<html>
<head>
${r}
</head>`) : i = `${r}
${t}`, { htmlContent: i, mimeType: n };
}
function u(e) {
  let s, t;
  if (e.content.type === "rawHtml") {
    ...
    if (s = e.content.htmlString, typeof s != "string")
      throw new Error(
        "MCP-UI SDK: content.htmlString must be provided as a string when content.type is 'rawHtml'."
      );
    if (e.adapters) {
      const n = m(s, e.adapters);
      s = n.htmlContent, t = n.mimeType ?? "text/html";
    } else
      t = "text/html";
    ...
    if (s = e.content.iframeUrl, typeof s != "string")
      throw new Error(
    ...
    if (s = e.content.script, typeof s != "string")
      throw new Error(
...
export {
  f as InternalMessageType,
  w as ReservedUrlParams,
  u as createUIResource,
  l as getAppsSdkAdapterScript,
  h as postUIActionResult,
  A as uiActionResultIntent,
  y as uiActionResultLink,
  S as uiActionResultNotification,
  M as uiActionResultPrompt,
  I as uiActionResultToolCall,
