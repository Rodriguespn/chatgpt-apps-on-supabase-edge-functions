  ...
  var MCPUIAppsSdkAdapter = class {
    ...
    sendRenderData(requestMessageId) {
      ...
      if (toolOutput !== void 0 && toolOutput !== null) {
        let parsedToolOutput = null;
        if (typeof toolOutput === "string") {
          try {
            parsedToolOutput = JSON.parse(toolOutput);
            this.config.logger.debug("[MCPUI-Apps SDK Adapter] Successfully parsed toolOutput as JSON");
          } catch (error) {
            this.config.logger.debug("[MCPUI-Apps SDK Adapter] toolOutput is not valid JSON, skipping merge");
          }
        } else if (typeof toolOutput === "object") {
          parsedToolOutput = toolOutput;
        }
        if (parsedToolOutput && typeof parsedToolOutput === "object" && !Array.isArray(parsedToolOutput)) {
          Object.assign(renderData, parsedToolOutput);
          this.config.logger.debug("[MCPUI-Apps SDK Adapter] Merged toolOutput into renderData");
        }
      }
